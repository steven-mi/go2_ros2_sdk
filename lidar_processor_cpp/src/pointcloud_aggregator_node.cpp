...
{
  this->declare_parameter("input_topic", "/pointcloud/current");
  this->declare_parameter("aggregation_mode", "instant");
  this->declare_parameter("temporal_window_sec", 1.0);
...
AggregatorConfig PointCloudAggregatorNode::loadConfiguration()
{
  ...
  config.input_topic = this->get_parameter("input_topic").as_string();
  config.aggregation_mode = this->get_parameter("aggregation_mode").as_string();
  config.temporal_window_sec = this->get_parameter("temporal_window_sec").as_double();
...
void PointCloudAggregatorNode::pointcloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  try {
    ...
    if (config_.aggregation_mode == "temporal") {
      aggregated_clouds_.push_back(filtered_cloud);
      cloud_timestamps_.push_back(std::chrono::steady_clock::now());

      const size_t max_clouds = static_cast<size_t>(
        std::max(1.0, config_.temporal_window_sec * config_.publish_rate));
      while (aggregated_clouds_.size() > max_clouds) {
        aggregated_clouds_.erase(aggregated_clouds_.begin());
        cloud_timestamps_.erase(cloud_timestamps_.begin());
      }
    }
...
void PointCloudAggregatorNode::publishCallback()
{
  try {
    ...
    if (config_.aggregation_mode == "temporal") {
      ...
      if (!has_latest_ || !latest_cloud_ || latest_cloud_->points.empty()) {
        return;
      }
      *combined_cloud = *latest_cloud_;
    }
    ...
    // Preserve source frame (typically odom); laserscan node transforms via TF.
    std_msgs::msg::Header header;
    header.stamp = this->get_clock()->now();
    header.frame_id = has_latest_ ? latest_header_.frame_id : "odom";
...
void PointCloudAggregatorNode::logConfiguration()
{
  ...
  RCLCPP_INFO(this->get_logger(), "   Input topic: %s", config_.input_topic.c_str());
  RCLCPP_INFO(this->get_logger(), "   Aggregation mode: %s", config_.aggregation_mode.c_str());
  if (config_.aggregation_mode == "temporal") {
    RCLCPP_INFO(this->get_logger(), "   Temporal window: %.1fs", config_.temporal_window_sec);
  }
