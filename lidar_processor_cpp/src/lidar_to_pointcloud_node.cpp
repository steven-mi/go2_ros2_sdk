...
void LidarToPointCloudNode::setupPublishers()
{
  ...
  current_frame_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
    "/pointcloud/current", qos
  );
}
...
void LidarToPointCloudNode::lidarCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  try {
    ...
    // Per-frame cloud for SLAM (no temporal ghosting)
    publishCurrentFrame(*msg);

    // Add to aggregator for 3D map / PLY export
    aggregator_->addPoints(points);

    // Publish cumulative point cloud for visualization
...
void LidarToPointCloudNode::publishCurrentFrame(const sensor_msgs::msg::PointCloud2& msg)
{
  current_frame_pub_->publish(msg);
}
